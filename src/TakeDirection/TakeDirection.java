...
to walk (eg. ['n', 's', 'w', 'e']). You always walk only a single block for each letter (direction) and you know it
takes you one minute to traverse one city block, so create a function that will return true if the walk the app gives
...
public class TakeDirection {
    public static void main(String[] args) {
        char[] directions = {'n', 's', 'n', 's', 'e', 'w', 'e', 'e', 'w'};
    ...
    public static boolean isValid(char[] walk) {
        ...
        for (char char1 : walk) {
            switch (char1) {
                case 'n' -> verticalDirection++;
                case 's' -> verticalDirection--;
                case 'e' -> horizontalDirection++;
                case 'w' -> horizontalDirection--;
            }
        }
