...
He tells them that he will only sit for the session if they show the same motif at most N times. Luckily, Alice and Bob
are able to encode the motif as a number. Can you help them to remove numbers such that their list contains each number
...
public class DeleteOccurrences {
    public static void main(String[] args) {
        int occ = 2;
        int[] elements = {1, 2, 1, 2, 1, 2, 30};
        System.out.println(Arrays.toString(deleteNth(elements, occ)));
    }

    public static int[] deleteNth(int[] elements, int maxOccurrences) {
        List<Integer> elementsList = new ArrayList<>();
        for (int singleElement : elements) {
            if (Collections.frequency(elementsList, singleElement) < maxOccurrences)
                elementsList.add(singleElement);
        }
        int[] finalElements = new int[elementsList.size()];
        for (int i = 0; i < elementsList.size(); i++) {
            finalElements[i] = elementsList.get(i);
        }
